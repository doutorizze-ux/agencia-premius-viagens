...
document.addEventListener('click', async (event) => {
  ...
  try {
    ...
    if (type === 'delete-excursion') {
      ...
      if (item && window.confirm(`Excluir a excursão “${item.title}”? Essa ação não pode ser desfeita.`)) {
        await api(`/api/excursions/${item.id}`, { method: 'DELETE' }); $('#modal-root').innerHTML = ''; await loadData(); toast('Excursão excluída'); renderExcursions();
      }
    }
...
document.addEventListener('submit', async (event) => {
  ...
  if (event.target.id === 'excursion-form') { event.preventDefault(); const input = Object.fromEntries(new FormData(event.target).entries()); const id = input.id; delete input.id; try { await api(id ? `/api/excursions/${id}` : '/api/excursions', { method: id ? 'PATCH' : 'POST', body: JSON.stringify(input) }); $('#modal-root').innerHTML = ''; await loadData(); toast(id ? 'Excursão atualizada' : 'Excursão criada com sucesso'); renderExcursions(); } catch (error) { toast(error.message, true); } }
