...
function renderExcursions() {
  const filters = ['Todos os roteiros', 'Vendas abertas', 'Encerrados'];
  const filtered = state.excursions.filter((item) => state.excursionFilter === 'Todos os roteiros' || (state.excursionFilter === 'Vendas abertas' ? item.status !== 'Encerrada' : item.status === 'Encerrada'));
  const cards = filtered.map((item) => `<article class="excursion-card"><div class="excursion-banner ${item.color}"><span>${esc(item.status)}</span></div><div class="excursion-card-body"><h3>${esc(item.title)}</h3><div class="destination">⌖ ${esc(item.destination)}</div><div class="excursion-meta"><span>Saída<b>${date(item.date)}</b></span><span>Investimento<b>${money(item.price)}</b></span></div><div class="excursion-footer"><div class="muted">${item.reserved} de ${item.seats} lugares<div class="progress-line"><i style="width:${Math.min(100, item.reserved / Math.max(1, item.seats) * 100)}%"></i></div></div><button class="secondary-button" style="padding:7px 9px;font-size:10px" data-action="edit-excursion" data-id="${item.id}">Detalhes</button></div></div></article>`).join('');
  const empty = `<div class="empty-card excursion-empty"><div><span>✦</span><strong>Nenhuma excursão encontrada</strong><p>Altere o filtro ou cadastre um novo roteiro.</p></div></div>`;
...
function renderCalendar() {
  ...
  for (let day = 1; day <= daysInMonth; day += 1) {
    ...
    cells.push(`<div class="calendar-day ${isToday ? 'today' : ''}"><span class="calendar-number">${day}</span><div class="calendar-events">${events.slice(0, 3).map((item) => `<button class="calendar-event ${item.color}" data-action="edit-excursion" data-id="${item.id}" title="${esc(item.title)}"><b>${esc(item.title.split(' · ')[0])}</b><small>${esc(item.destination)}</small></button>`).join('')}${events.length > 3 ? `<span class="calendar-more">+${events.length - 3} roteiros</span>` : ''}</div></div>`);
  }
...
function openExcursionModal(item = null) {
  ...
  $('#modal-root').innerHTML = `<div class="modal-backdrop"><form class="modal" id="excursion-form"><div class="modal-header"><h2>${editing ? 'Editar excursão' : 'Nova excursão'}</h2><button type="button" data-action="close-modal">×</button></div><div class="modal-body"><input type="hidden" name="id" value="${value('id')}" /><label class="form-label">Nome do roteiro<input class="form-control" name="title" value="${value('title')}" placeholder="Ex.: Serra da Canastra · Feriado" required /></label><label class="form-label">Destino<input class="form-control" name="destination" value="${value('destination')}" placeholder="Cidade / estado" required /></label><div class="form-grid"><label class="form-label">Data de saída<input class="form-control" type="date" name="date" value="${value('date')}" required /></label><label class="form-label">Data de retorno<input class="form-control" type="date" name="endDate" value="${value('endDate', item?.date || '')}" required /></label></div><div class="form-grid"><label class="form-label">Valor por pessoa<input class="form-control" type="number" name="price" value="${value('price')}" placeholder="690" min="0" required /></label><label class="form-label">Lugares<input class="form-control" type="number" name="seats" value="${value('seats', 40)}" placeholder="40" min="1" required /></label></div><div class="form-grid"><label class="form-label">Status<select class="form-control" name="status"><option ${item?.status === 'Vendas abertas' || !item ? 'selected' : ''}>Vendas abertas</option><option ${item?.status === 'Quase lotada' ? 'selected' : ''}>Quase lotada</option><option ${item?.status === 'Confirmada' ? 'selected' : ''}>Confirmada</option><option ${item?.status === 'Encerrada' ? 'selected' : ''}>Encerrada</option></select></label><label class="form-label">Cor do card<select class="form-control" name="color"><option value="lime" ${item?.color === 'lime' ? 'selected' : ''}>Verde</option><option value="blue" ${item?.color === 'blue' || !item ? 'selected' : ''}>Azul</option><option value="orange" ${item?.color === 'orange' ? 'selected' : ''}>Laranja</option><option value="purple" ${item?.color === 'purple' ? 'selected' : ''}>Roxo</option></select></label></div><label class="form-label">Descrição curta<textarea class="form-control" name="description" rows="3" placeholder="O que está incluso no pacote?">${value('description')}</textarea></label></div><div class="modal-footer">${editing ? `<button type="button" class="danger-button" data-action="delete-excursion" data-id="${item.id}">Excluir</button>` : ''}<button type="button" class="secondary-button" data-action="close-modal">Cancelar</button><button type="button" class="primary-button" data-action="save-excursion">${editing ? 'Salvar alterações' : 'Salvar excursão'}</button></div></form></div>`;
}
...
document.addEventListener('click', async (event) => {
  ...
  const excursionFilter = event.target.closest('[data-excursion-filter]'); if (excursionFilter) { state.excursionFilter = excursionFilter.dataset.excursionFilter; renderExcursions(); return; }
  ...
  try {
    ...
    if (type === 'edit-excursion') { const item = state.excursions.find((excursion) => excursion.id === action.dataset.id); if (item) openExcursionModal(item); }
    if (type === 'delete-lead') { const lead = state.leads.find((item) => item.id === action.dataset.leadId); if (lead) openDeleteLeadConfirm(lead); }
    if (type === 'confirm-delete-lead') { const lead = state.leads.find((item) => item.id === action.dataset.id); if (lead) { action.disabled = true; action.textContent = 'Excluindo…'; await api(`/api/leads/${lead.id}`, { method: 'DELETE' }); $('#modal-root').innerHTML = ''; state.selectedLead = null; state.conversation = []; await loadData(); toast('Lead excluído'); renderLeads(); } }
    if (type === 'delete-excursion') {
      const item = state.excursions.find((excursion) => excursion.id === action.dataset.id);
      if (item) openDeleteExcursionConfirm(item);
    }
    ...
    if (type === 'export-excursions') {
      ...
      const link = document.createElement('a'); link.href = URL.createObjectURL(new Blob([`\ufeff${csv}`], { type: 'text/csv;charset=utf-8' })); link.download = `excursoes-premius-${new Date().toISOString().slice(0, 10)}.csv`; link.click(); URL.revokeObjectURL(link.href); toast('Excursões exportadas');
    }
    ...
    if (type === 'refresh-reports') { action.disabled = true; await loadData(); renderReports(); toast('Relatório atualizado'); }
